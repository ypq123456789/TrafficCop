...
# 修改：更新 add_to_crontab 函数
add_to_crontab() {
    (crontab -l 2>/dev/null; echo "* * * * * RUN_MODE=check $SCRIPT_PATH >> $CRON_LOG 2>&1") | crontab -
    if [ "$DAILY_REPORT" = "true" ]; then
        (crontab -l 2>/dev/null; echo "0 0 * * * RUN_MODE=daily_report $SCRIPT_PATH >> $CRON_LOG 2>&1") | crontab -
    fi
...
main() {
    ...
    # 修改：使用 RUN_MODE 环境变量替代 \$1
    if [ "$RUN_MODE" = "daily_report" ]; then
        daily_report
    else
